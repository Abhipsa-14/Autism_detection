...
async def model_info():
    """Public, non-sensitive model card for the 'How it works' UI section."""
    meta = model_service.metadata or {}
    tm = meta.get("test_metrics", {}) or {}
    shap_imp = meta.get("shap_global_importance", {}) or {}
    # Top questionnaire drivers (A*_Score only), normalised 0-1 for display
    aq_items = {k: v for k, v in shap_imp.items() if k.startswith("A") and k.endswith("_Score")}
    top = sorted(aq_items.items(), key=lambda x: -x[1])[:5]
    max_v = max(aq_items.values(), default=1) or 1
    return {
        ...
        "metrics": {
            ...
            "sensitivity_recall": tm.get("sensitivity_recall"),
            "specificity": tm.get("specificity"),
            "precision": tm.get("precision"),
        },
        "top_factors": [
            {"feature": k, "weight": round(v / max_v, 3)} for k, v in top
        ],
    }
...
async def predict(
    ...
    # Attach live explainability fields (computed at inference, not persisted)
    response = PredictionResult.model_validate(pred)
